...
async def pause(client: Client, message: Message):
    ...
    await message.reply_text("✯Imteyaz✯=⏸ Paused.")
...
async def resume(client: Client, message: Message):
    ...
    await message.reply_text("✯Imteyaz✯=▶️ Resumed.")
...
async def stop(client: Client, message: Message):
    ...
    await message.reply_text("✯Imteyaz✯=⏹ Stopped streaming.")
...
async def skip(client: Client, message: Message):
    ...
    if sira.is_empty(chat_id):
        ...
        await message.reply_text("✯Imteyaz✯=❗ Nothing in queue")
    else:
        tgcalls.pytgcalls.change_stream(
            chat_id, sira.get(chat_id)["file_path"]
        )

        await message.reply_text("✯Imteyaz✯=⏩ Skipped the current song.")
...
async def admincache(client, message: Message):
    ...
    await message.reply_text("✯Imteyaz✯=❇️ Admin cache refreshed!")
